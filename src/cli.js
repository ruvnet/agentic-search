import {hybrid} from './ruvector.js';
...
try{
 ...
 const corpus=readFileSync(process.env.SEARCH_CORPUS||new URL('../fixtures/corpus.json',import.meta.url));if(corpus.length>17*1024*1024)throw Error('Corpus too large');const engine=process.env.SEARCH_MODE==='hybrid'?await hybrid(JSON.parse(corpus)):index(JSON.parse(corpus));if(engine.close)process.on('exit',()=>engine.close());
 ...
 else if(command==='search')console.log(JSON.stringify(await engine.search(process.argv[3])));
 else if(command==='benchmark'){const timings=[];for(let i=0;i<1000;i++){const start=performance.now();await engine.search('signed federation');timings.push(performance.now()-start);}timings.sort((a,b)=>a-b);console.log(JSON.stringify({fixtureDocuments:engine.size,iterations:1000,p50ms:timings[500],p95ms:timings[950],productionClaim:false}));}
